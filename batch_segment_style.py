...
for style_image in style_files:
    for content_image in content_files:
        ...
        with Image.open(output_image_path) as res_im:
            d = res_im.resize((og_width, og_height), resample=Image.BOX)
            d.save(output_image_path_upscaled)

        # os.system(f"convert -resize {args.percent_upscaled_result}% {output_image_path} {output_image_path_upscaled}")
